package com.example.tidus.ristrat.weight;
...
public class LineChart extends View {
    ...
    private float mPointRadius = 5;//点的半径
    private int mDateTextColor = Color.parseColor("#000000");//日期字体颜色
    private int mDarkColor = Color.parseColor("#5b7fdf");//点、线的颜色(深色)
    private int mLightColor = Color.parseColor("#d5d8f7");//点、线的颜色(浅色)
    private int mPointColor = Color.parseColor("#d5d8f7");//点、线的颜色(浅色)
    private int mShapeColor = Color.parseColor("#ffffff");//阴影的颜色
    ...
    public LineChart(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            mPointColor = typedArray.getColor(R.styleable.LineChart_PointColor, mPointColor);
...
//        valueAnimator.setRepeatMode(ValueAnimator.RESTART);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mXItems == null) {
            mXItems = new String[]{"日", "一", "二", "三", "四", "五"};
            mPoints = new int[]{0, -1, -1, -1, -1, -1};
        ...
        //原点坐标
        xOrigin = (int) (0.5 * (mWidth / mLength) - mFontSize / 2);
        yOrigin = (int) (max * ((mHeight - mLength * mFontSize) / max) + 4 * mFontSize);
...
//                    , xPoints[i] - mFontSize / 4, yPoints[i] - mFontSize, mPointPaint);
        ...
        for (int i = 0; i < mLength; i++) {
            if (mPoints[i] == -1) {
                mLinePaint.setColor(mLightColor);
                mPointPaint.setColor(mPointColor);
            } else if (mPoints[i] <= 1 && mPoints[i] > 0) {
                mLinePaint.setColor(mDarkColor);
                mPointPaint.setColor(Color.GREEN);
            } else if (mPoints[i] <= 2 && mPoints[i] > 1) {
                mLinePaint.setColor(mDarkColor);
                mPointPaint.setColor(Color.BLUE);
            } else if (mPoints[i] <= 3 && mPoints[i] > 2) {
                mLinePaint.setColor(mDarkColor);
                mPointPaint.setColor(Color.YELLOW);
            } else if (mPoints[i] <= 4 && mPoints[i] > 3) {
                mLinePaint.setColor(mDarkColor);
                mPointPaint.setColor(Color.RED);
            } else if (mPoints[i] <= 5 && mPoints[i] > 4) {
                mLinePaint.setColor(mDarkColor);
                mPointPaint.setColor(Color.BLACK);
            }
            //画点
